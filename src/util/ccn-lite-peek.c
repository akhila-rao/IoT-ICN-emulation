...
usage:
    ...
    for (cnt = 0; cnt <= MAX_NETWORK_LEVEL_INTEREST_RETRANSMIT; cnt++) {
        int nonce = random();
        int rc;
        ...
        for (;;) { // wait for a content pkt (ignore interests)
            ...
            int enc, suite2, len2;
            fprintf(stderr, "ICNIoT: Interest sent from node %d\n",(port-9000));
            ...
            write(1, out, len);
            fprintf(stderr, "ICNIoT: ccn-lite-peek recieved CO at node %d\n",(port-9000));
            myexit(0);
        }
        if (cnt < MAX_NETWORK_LEVEL_INTEREST_RETRANSMIT)
            DEBUGMSG(WARNING, "re-sending interest\n");
    }
    fprintf(stderr, "timeout after retransmitting %d times\n",MAX_NETWORK_LEVEL_INTEREST_RETRANSMIT);
