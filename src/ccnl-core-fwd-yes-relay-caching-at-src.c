...
//    DEBUGMSG_CFWD(INFO, " ICNIoT: incoming_data=<%s>%s from=%s\n",
//                  ccnl_prefix_to_path((*pkt)->pfx),
//                  ccnl_suite2str((*pkt)->suite),
//                  ccnl_addr2ascii(from ? &from->peer : NULL));
      DEBUGMSG_CFWD(INFO, " ICNIoT: incoming_data %s, from=%d\n",ccnl_prefix_to_path((*pkt)->pfx),from->ifndx);
...
//  FIXME: mgmt messages for NDN and other suites?
    ...
    if (relay->max_cache_entries != 0) { // it's set to -1 or a limit. DONOT use -1 now since I am using size in a lot of decisions
        DEBUGMSG_CFWD(DEBUG, " ICNIoT: caching_request_from_face %d, ifndx %d\n",from->faceid,from->ifndx);

// if(from->ifndx != 1){//if this is NOT a self published CO
...
//1 - (OCCUPANCY_WEIGHT* +EXPIRY_TIME_WEIGHT*)
    ...
    if(rand()%100 < cache_prob){
        ...
        DEBUGMSG_CFWD(DEBUG, " ICNIoT: USE_RESOURCE_CACHE_STRATEGY: content not cached\n");
        free_content(c);
       }
#elif defined(USE_PROB_CACHE_STRATEGY)
DEBUGMSG_CFWD(DEBUG, " ICNIoT: USE_PROB_CACHE_STRATEGY\n");
    if((rand()%100 < relay->cache_prob) && (relay->contentcnt <= relay->max_cache_entries)){
         DEBUGMSG_CFWD(DEBUG, " ICNIoT: USE_PROB_CACHE_STRATEGY: need to cache\n");
         ccnl_content_add2cache(relay, c);
         DEBUGMSG_CFWD(DEBUG, " ICNIoT: USE_PROB_CACHE_STRATEGY: done caching\n");
      }
    else {
         DEBUGMSG_CFWD(DEBUG, " ICNIoT: content not cached\n");
         free_content(c);
      }
#else 
    DEBUGMSG_CFWD(DEBUG, " ICNIoT: NO_CACHE_STRATEGY\n");
    ccnl_content_add2cache(relay, c);
#endif 
 }
 else {//self published CO
      DEBUGMSG_CFWD(DEBUG, " ICNIoT: contentObject to be published\n");
      ccnl_content_add2cache(relay, c);
      DEBUGMSG_CFWD(DEBUG, " ICNIoT: contentObject_published\n");
   }
    } else {
...
// being cached. 
        ...
        if (from->ifndx >= 0) {
            DEBUGMSG_CFWD(INFO, " ICNIoT: outgoing_data\n");
