...
//#ifdef USE_SIGNATURES
        ...
        else
        {
            ...
            else
            {
                ...
                ccnl_content_serve_pending(ccnl, c, from);
...
Bail:
    ...
    len3 = ccnl_ccnb_mkHeader(fwdentry_buf, CCNL_DTAG_PREFIX, CCN_TT_DTAG);
    len3 += ccnl_ccnb_mkStrBlob(fwdentry_buf+len3, CCN_DTAG_ACTION, CCN_TT_DTAG, cp);
    bufpt = ccnl_prefix_to_path(p); 
    len3 += ccnl_ccnb_mkStrBlob(fwdentry_buf+len3, CCN_DTAG_NAME, CCN_TT_DTAG, bufpt); // prefix
    ccnl_free(bufpt);
    len3 += ccnl_ccnb_mkStrBlob(fwdentry_buf+len3, CCN_DTAG_FACEID, CCN_TT_DTAG, (char*) faceid);
    memset(h,0,sizeof(h));
    sprintf((char*)h, "%d", (int)suite[0]);
                    ...
                    struct ccnl_prefix_s *prefix, struct ccnl_face_s *from)
{
    ...
    char* bufpo = NULL;
    struct ccnl_prefix_s *prefixptr = NULL;
    ...
    prefixptr = ccnl_prefix_dup(prefix_new);
    bufpo = ccnl_prefix_to_path(prefixptr);
    DEBUGMSG(TRACE, "  mgmt: adding object %s to cache (suite=%s)\n",
             bufpo, ccnl_suite2str(suite));
    ccnl_free(bufpo);
    free_prefix(prefixptr);
    //Reply MSG
    if (h)
        ccnl_free(h);
    h = ccnl_malloc(300);
    bufpo = ccnl_prefix_to_path(prefix_new);
    sprintf((char *)h, "received add to cache request, inizializing callback for %s", bufpo);
    ccnl_free(bufpo);
