...
#ifdef USE_RESOURCE_CACHE_STRATEGY
    double cache_prob=0;
#endif
      DEBUGMSG_CORE(INFO, " ICNIoT: incoming_data, from=%s\n",ccnl_addr2ascii(from ? &from->peer : NULL));
...
//  FIXME: mgmt messages for NDN and other suites?
    ...
    if (!ccnl_content_serve_pending(relay, c, from)) { // unsolicited content
        ...
        // self published CO s do not enter here
        DEBUGMSG_CFWD(DEBUG, "  removed because no matching interest\n");
        free_content(c);
        return 0;
    }
    if (relay->max_cache_entries != 0) { // it's set to -1 or a limit. DONOT use -1 now since I am using size in a lot of decisions
//        DEBUGMSG_CORE(INFO, " ICNIoT: caching_request_from_face %d, ifndx %d\n",from->faceid,from->ifndx);

   if(!(c->flags & CCNL_CONTENT_FLAGS_STATIC)){//if this is NOT a self published CO
#ifdef USE_RESOURCE_CACHE_STRATEGY
//1 - (OCCUPANCY_WEIGHT* +EXPIRY_TIME_WEIGHT*)
    if(relay->contentcnt == relay->max_cache_entries)
         cache_prob=(1-OCCUPANCY_WEIGHT*((double)((relay->contentcnt)-1)/(double)(relay->max_cache_entries)))*100;
    else if (relay->contentcnt < relay->max_cache_entries)
         cache_prob=(1-OCCUPANCY_WEIGHT*((double)(relay->contentcnt)/(double)(relay->max_cache_entries)))*100;
    else cache_prob=0;

    if(rand()%100 < cache_prob){
         ccnl_content_add2cache(relay, c);
     }
    else {
        DEBUGMSG_CORE(INFO, " ICNIoT: USE_RESOURCE_CACHE_STRATEGY: content not cached\n");
        free_content(c);
       }
#elif defined(USE_PROB_CACHE_STRATEGY)
DEBUGMSG_CORE(INFO, " ICNIoT: USE_PROB_CACHE_STRATEGY\n");
    if((rand()%100 < relay->cache_prob) && (relay->contentcnt <= relay->max_cache_entries)){
//         DEBUGMSG_CORE(INFO, " ICNIoT: USE_PROB_CACHE_STRATEGY: need to cache\n");
         ccnl_content_add2cache(relay, c);
//         DEBUGMSG_CORE(INFO, " ICNIoT: USE_PROB_CACHE_STRATEGY: done caching\n");
      }
    else {
         DEBUGMSG_CORE(INFO, " ICNIoT: content not cached\n");
         free_content(c);
      }
#else 
    DEBUGMSG_CORE(INFO, " ICNIoT: NO_CACHE_STRATEGY\n");
    ccnl_content_add2cache(relay, c);
#endif 
 }
 else {//self published CO and non-self published caching is allowed
//      DEBUGMSG_CORE(INFO, " ICNIoT: contentObject to be published\n");
      ccnl_content_add2cache(relay, c);
      DEBUGMSG_CORE(INFO, " ICNIoT: contentObject_published\n");
   }
    } else {
        if(c->flags & CCNL_CONTENT_FLAGS_STATIC){//self published CO and non-self published caching is not allowed
//           DEBUGMSG_CORE(INFO, " ICNIoT: contentObject to be published\n");
           ccnl_content_add2cache(relay, c);
           DEBUGMSG_CORE(INFO, " ICNIoT: contentObject_published\n");
        }
        else{
        DEBUGMSG_CFWD(DEBUG, "  content not added to cache, this node does not cache\n");
        free_content(c);
         }
    }
    return 0;
                        ...
                        struct ccnl_pkt_s **pkt, cMatchFct cMatch)
{
    ...
    char* delim="/";
    int interest_from_port=0;

    if (ccnl_nonce_isDup(relay, *pkt)) {
        DEBUGMSG_CFWD(DEBUG, " ICNIoT: dropped because of duplicate nonce\n");
        return 0;
    }
...
#endif
    prefix_buf = ccnl_prefix_to_path((*pkt)->pfx);
    interest_from_port = findIntFromStr(ccnl_addr2ascii(from ? &from->peer : NULL),delim);

    if(interest_from_port > 10000){// this interest is coming from ccn-lite-peek and hence arrives from a high numbered port
        DEBUGMSG_CFWD(INFO, " ICNIoT: consumer_incoming_interest=<%s> from=%s\n",
                  prefix_buf,
//                  ccnl_suite2str((*pkt)->suite),
                  ccnl_addr2ascii(from ? &from->peer : NULL));
    }
    else{//this interest is coming from a neighbouring relay
        DEBUGMSG_CFWD(INFO, " ICNIoT: relay_incoming_interest=<%s> from=%s\n",
                  prefix_buf,
//                  ccnl_suite2str((*pkt)->suite),
                  ccnl_addr2ascii(from ? &from->peer : NULL));
   ...
   // Dropping fo a pkt is based on a 2 state MArkov chaiin process with states active and outage
   // When in active it has active_linkErrorProb drop prob. And when in outage it has outage_linkErrorProb. 
    if((interest_from_port <= 10000) && drop_this_pkt(relay, from)){//to deactivate using lossy links set the drop probabilities of active and outage states to 0
        DEBUGMSG_CORE(INFO, " ICNIoT: Int_pkt dropped\n");
        return 0;
    }
        DEBUGMSG_CORE(INFO, " ICNIoT: Int_pkt NOT_dropped\n");
    // Step 1: search in content store
    DEBUGMSG_CFWD(DEBUG, " ICNIoT: searching in CS\n");
    for (c = relay->contents; c; c = c->next) {
        ...
        DEBUGMSG_CFWD(DEBUG, " ICNIoT: found matching content %p\n", (void *) c);
// last used must be updated if the content is popular and requested after
// being cached. 
...
//    DEBUGMSG_CFWD(DEBUG, " ICNIoT: ccnl_ccntlv_forwarder: %dB from face=%p (id=%d.%d)\n",
//                  *datalen, (void*)from, relay->id, from ? from->faceid : -1);
...
// function to parse a string and extract the number included after a delimiter in int form
int findIntFromStr(char * s1, char* delim){
...
int num=0;
int flag=0;
int strLen = strlen(s1);
for(i=0;i<strLen-strlen(delim);i++){
    if(strncmp(s1,delim,strlen(delim)) ==0){
        numStr=s1+strlen(delim);
        flag=1;
        break;
    }
    s1++;
...
else printf("Could not find the delimiter\n");
return num;
}
