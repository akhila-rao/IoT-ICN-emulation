...
ccnl_relay_config(struct ccnl_relay_s *relay, char *ethdev,
                  ...
                  char *uxpath, int nodeID, int suite, int max_cache_entries, int cache_prob, 
                  double linkErrProb[2], int last_state, int network_links[], int num_of_links,
                  char *crypto_face_path)
{
    ...
    relay->cache_prob = cache_prob;
    relay->linkErrProb[0] = linkErrProb[0];
    relay->linkErrProb[1] = linkErrProb[1];
    relay->last_pkt_recv_time = CCNL_NOW();
    relay->last_state = last_state;
    relay->nodeID = nodeID;
    relay->network_links_ptr = network_links;
    relay->num_of_links = num_of_links;
    int j=0;
    int *ptr = relay->network_links_ptr;
    printf("number of links = %d\n",num_of_links);
    for(j=0;j<num_of_links;j++){
        fprintf(stdout, " ICNIoT: CHECKING !!! Lossy link between %d and %d\n",*(ptr + 2*j), *(ptr + 2*j +1));
        fflush(stdout);
    }
...
main(int argc, char **argv)
{
    ...
    int cache_prob = CACHE_PROB;
    double linkErrProb[2] = {ACTIVE_DROP_PROB, OUTAGE_DROP_PROB};
    ...
    srandom(time(NULL));
    FILE *fp = fopen("../scripts/cacheSize.txt", "r");
    if (fp == NULL) {
       fprintf(stdout,"Can't open input file cacheSize.txt!\n");
       exit(1);
     }
    fscanf(fp, "%d", &max_cache_entries);
    fprintf(stdout, "ICNIoT: cacheSize:%d\n",max_cache_entries);
    fclose(fp);

    FILE *fp2 = fopen("../scripts/cacheProb.txt", "r");
    if (fp2 == NULL) {
       fprintf(stdout, "Can't open input file cacheProb.txt!\n");
       exit(1);
     }
    fscanf(fp2, "%d", &cache_prob);
    fprintf(stdout, "ICNIoT: caching probability:%d\n",cache_prob);
    fclose(fp2);

    FILE *fp3 = fopen("../scripts/active_linkErrorProb.txt", "r");
    if (fp3 == NULL) {
       fprintf(stdout, "Can't open input file active_linkErrorProb.txt!\n");
       exit(1);
     }
    fscanf(fp3, "%lf", &linkErrProb[0]);
    fprintf(stdout, " ICNIoT: Active link error probability:%f\n",linkErrProb[0]);
    fclose(fp3);   
    fflush(stdout);

    FILE *fp4 = fopen("../scripts/outage_linkErrorProb.txt", "r");
    if (fp4 == NULL) {
       fprintf(stdout, "Can't open input file outage_linkErrorProb.txt!\n");
       exit(1);
     }
    fscanf(fp4, "%lf", &linkErrProb[1]);
    fprintf(stdout, " ICNIoT: Outage link error probability:%f\n",linkErrProb[1]);
    fclose(fp4);
    fflush(stdout);

    FILE *fp5 = fopen("../scripts/lossyLinks.txt", "r");
    char ch;
    int lines=0;
    if (fp5 == NULL) {
       fprintf(stdout, "Can't open input file lossyLinks.txt!\n");
       exit(1);
     }
    while(!feof(fp5)){
        ch = fgetc(fp5);
    ...
    rewind(fp5);
    int i=0;
    int network_links[lines*2];
    for(i=0;i<lines*2;i++){
        fscanf(fp5, "%d",&network_links[i]);
    }
    fclose(fp5);
    fflush(stdout);

    for(i=0;i<lines;i++){
        fprintf(stdout, " ICNIoT: Lossy link between %d and %d\n",network_links[2*i], network_links[2*i +1]);
    }
...
//    DEBUGMSG(INFO, "using suite %s\n", ccnl_suite2str(suite));
    printf("num of lines in main is %d\n", lines);
    ccnl_relay_config(&theRelay, ethdev, udpport1, udpport2, httpport,
                      uxpath, nodeID, suite, max_cache_entries, cache_prob, 
		      linkErrProb, last_state, network_links, lines, crypto_sock_path);
    if (datadir)
