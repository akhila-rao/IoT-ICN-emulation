        ...
        if (!i->from || fwd->face != i->from ||
            ...
            DEBUGMSG_CFWD(INFO, "  outgoing interest=<%s> to=%s\n",
                          ...
                          fwd->face ? ccnl_addr2ascii(&fwd->face->peer)
                                    : "<tap>");
            DEBUGMSG_CFWD(INFO, " ICNIoT: outgoing_interest\n");
...
//    free_content(c);
    ...
    ccnl->contentcnt--;
    DEBUGMSG_CORE(TRACE, " ICNIoT: remove_contentFromCache:%d\n",ccnl->contentcnt);
    return c2;
}
...
ccnl_content_add2cache(struct ccnl_relay_s *ccnl, struct ccnl_content_s *c)
{
    ...
    if (c->flags & CCNL_CONTENT_FLAGS_STATIC){
        DEBUGMSG_CORE(TRACE, " ICNIoT: going to add self published content to cache \n");
        DBL_LINKED_LIST_ADD(ccnl->contents, c);
        DEBUGMSG_CORE(TRACE, " ICNIoT: added_content2cache without incrementing count:%d\n",ccnl->contentcnt);
        return c;
     }
    ...
    if (ccnl->max_cache_entries > 0 &&
        ...
        DEBUGMSG_CORE(TRACE, " ICNIoT: try to remove oldest content\n");
        struct ccnl_content_s *c2;
        struct ccnl_content_s *c3;
        int age = 0;
//        int flag=0;
...
//          if(flag==1){
            ...
            DEBUGMSG_CORE(TRACE, " ICNIoT: yes ! removed oldest content\n");
//         }
//         else{
//            DEBUGMSG_CORE(TRACE, " ICNIoT: No ! could not remove any content objects from CS\n");
//         }
    }
    if(ccnl->contentcnt < ccnl->max_cache_entries){
        DEBUGMSG_CORE(TRACE, " ICNIoT: going to add content to cache \n");
    	DBL_LINKED_LIST_ADD(ccnl->contents, c);
    	ccnl->contentcnt++;
    	DEBUGMSG_CORE(TRACE, " ICNIoT: added_content2cache count:%d\n",ccnl->contentcnt);
    }
    return c;
}
...
ccnl_content_serve_pending(struct ccnl_relay_s *ccnl, struct ccnl_content_s *c)
{
    ...
    for (i = ccnl->pit; i;) {
		...
		DEBUGMSG_CFWD(INFO, " ICNIoT: outgoing_data on_faceid%d \n",pi->face->faceid);
...
ccnl_do_ageing(void *ptr, void *dummy)
{
    ...
    time_t t = CCNL_NOW();
    DEBUGMSG_CORE(TRACE, " ICNIoT: ageing t=%d\n", (int)t);

    while (c) {
        if ((c->last_used + CCNL_CONTENT_TIMEOUT) <= t &&
                                !(c->flags & CCNL_CONTENT_FLAGS_STATIC)){
          DEBUGMSG_CORE(TRACE, " ICNIoT: AGING: CONTENT REMOVE %p\n", (void*) c);
...
//akhila 29-10-2015
        // check if it is the n th time instant. If yes then save the content store 
             if (((int)t)%CS_SAVE_PERIOD == 0){
                 DEBUGMSG_CORE(TRACE, " ICNIoT: ContentStoreItem :%s\n",ccnl_prefix_to_path((c->pkt)->pfx));
             }
            c = c->next;
        }
    }
    DEBUGMSG_CORE(TRACE, " ICNIoT: ContentStoreItem contentCount:%d\n",relay->contentcnt);
    while (i) { // CONFORM: "Entries in the PIT MUST timeout rather
                // than being held indefinitely."
        if ((i->last_used + CCNL_INTEREST_TIMEOUT) <= t ||
                                i->retries > CCNL_MAX_INTEREST_RETRANSMIT) {
            DEBUGMSG_CORE(TRACE, " ICNIoT: AGING: INTEREST REMOVE %p\n", (void*) i);
            ...
            DEBUGMSG_CORE(DEBUG, " ICNIoT: retransmit %d <%s>\n", i->retries,
                     ccnl_prefix_to_path(i->pkt->pfx));
#ifdef USE_NFN
            if (i->flags & CCNL_PIT_COREPROPAGATES){
#endif
                DEBUGMSG_CORE(TRACE, " ICNIoT: AGING: PROPAGATING INTEREST %p\n", (void*) i);
    ...
    DEBUGMSG_CORE(TRACE, " ICNIoT: PITcount %d\n",relay->pitcnt);
    while (f) {
