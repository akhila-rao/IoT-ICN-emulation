...
//akhila 27-10-2015
#define USE_PROB_CACHE_STRATEGY 1
#define USE_LOSSY_LINKS  1
#define UPDATE_TIME_SERIES 1
//#define RECOMMENDED_CACHE_TIME 1
//#define USE_RESOURCE_CACHE_STRATEGY 1
...
struct ccnl_relay_s {
    ...
    time_t startup_time;
#endif
    int id;//possibly useless, but is being used in prints where its value is alwaus 0
    int nodeID;
...
//akhila 16-10-2015
    ...
    int cache_prob;             // 100 for all caching
    double linkErrProb[2];
//    int active_linkErrorProb;          // in percentage 0 for lossless links
//    int outage_linkErrorProb;          // in percentage 0 for lossless links
    double last_pkt_recv_time;   // the time stamp when the previous pkt was received. Needed to maintain state for correlated lossy links
    int last_state;             // can take values (0->active) or (1->outage) the state (active or outage) when the previous pkt was received. Needed to maintain state for correlated lossy links
