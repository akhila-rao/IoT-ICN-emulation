...
#folders=["highTraffic_topologyBA1_lossless_probability_based_caching_LRU/","highTraffic_topologyBA1_lossless_uniformRandom_LRU/"]
#folders=["highTraffic_topologyBA1_lossless_probability_based_caching_UTS_LRU/", "highTraffic_topologyBA1_lossless_probability_based_caching_LRU/"]
#folders=["lossless_lowTraffic_topologyBA1_LRU_probability_based_caching/", "lossless_lowTraffic_topologyBA1_UTS_LRU/"]
folders=["lossless_lowTraffic_topologyBA1_UTS_LRU/", "lossless_lowTraffic_topologyBA1_LRU_probability_based_caching/"]
...
if PLOT_NUM_OF_PKTS:
	p, ax = plt.subplots()
        p1 = ax.errorbar(CSsizes, avgNumPkts[0,:]/1000, yerr=errorBar[0,:]/1000, fmt='rd-', linewidth=5.0, ms=20.0, markeredgewidth=2, markeredgecolor='r', markerfacecolor='r')
        p2 = ax.errorbar(CSsizes, avgNumPkts[1,:]/1000, yerr=errorBar[1,:]/1000, fmt='bo--', linewidth=5.0, ms=20.0, markeredgewidth=2, markeredgecolor='b', markerfacecolor='None')

	plt.rcParams.update({'font.size': 30})
	plt.xlabel('Cache size (number of content objects)')
	plt.ylabel('Number of messages transmitted (X1000)')
#	plt.title('Time series content requests')
	plt.title('Cache all')
#        plt.legend([p1, p2], ['Time series requests', 'Uniform random requests'], loc='best')
        plt.legend([p1, p2], ['UTS-LRU', 'LRU'], loc='best')
	plt.xlim([0, 30])
	plt.ylim([0,170])
	plt.grid()
...
if PLOT_RECEPTION_RATE:
	r, axr = plt.subplots()
	r1 = axr.errorbar(CSsizes, avgReceptionRate[0,:], yerr=avgReceptionRateErrorBar[0,:], fmt='kD-', linewidth=3.0, ms=10.0)
	r2 = axr.errorbar(CSsizes, avgReceptionRate[1,:], yerr=avgReceptionRateErrorBar[1,:], fmt='ro-', linewidth=3.0, ms=10.0)
        r3 = axr.errorbar(CSsizes, avgReceptionRate[2,:], yerr=avgReceptionRateErrorBar[2,:], fmt='cx-', linewidth=3.0, ms=10.0)

        r4 = axr.errorbar(CSsizes, avgReceptionRate[3,:], yerr=avgReceptionRateErrorBar[3,:], fmt='ks--', linewidth=3.0, ms=10.0)
	r5 = axr.errorbar(CSsizes, avgReceptionRate[4,:], yerr=avgReceptionRateErrorBar[4,:], fmt='r^--', linewidth=3.0, ms=10.0)
	r6 = axr.errorbar(CSsizes, avgReceptionRate[5,:], yerr=avgReceptionRateErrorBar[5,:], fmt='c*--', linewidth=3.0, ms=10.0)
	plt.rcParams.update({'font.size': 30})
	plt.ylabel('Reception rate')
        plt.xlabel('Content store size (number of content objects)')
#        plt.title('Time series content requests')
#        plt.legend([p1, p2, p3, p4, p5, p6], ['LRU: Cache prob. 100%', 'LRU: Cache prob. 70%', 'LRU: Cache prob. 50%',
#        'UTS-LRU: Cache prob. 100%', 'UTS-LRU: Cache prob. 70%', 'UTS-LRU: Cache prob. 50%'], loc='best')
        plt.title('LRU')
        plt.legend([p1, p2, p3, p4, p5, p6], ['Time series requests: Cache prob. 100%', 'Time series requests: Cache prob. 70%', 
        'Time series requests: Cache prob. 50%', 'Uniform random requests: Cache prob. 100%', 
        'Uniform random requests: Cache prob. 70%', 'Uniform random requests: Cache prob. 50%'], loc='best')
#	plt.xlim([-5, 55])
#	plt.ylim([0.85, 1.0])
	plt.grid()
